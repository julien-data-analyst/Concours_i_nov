...
# Ajout de la lecture des données (dans le cas de modification)
dataset_i_nov = read.csv(file = "../Data/i_nov_vag_1_10_somm.csv", 
                         sep = ";",
                         quote = "\"",
                         header = TRUE)

dataset_i_nov_7_10 = dataset_i_nov %>% filter(VAGUE %in% c(7:10))
dataset_i_nov_1_6 = dataset_i_nov %>% filter(VAGUE %in% c(1:6))
