...
# Regardons les cas particuliers qui sont vides (présence de la région mais pas du département)
data_proj_1_3_V2 %>% 
  filter(DEPT == "")
...
##########################################################-
## Gérer les cas où ils n'ont "" ou NA.
## Traiter les lignes particulières 
ligns_part = dataset_inf_proj_1_3 %>% 
  filter((is.na(LOCALISATION) | is.na(`MONTANT DU PROJET`)))

# Vérification du succès de l'opération
ligns_part
## Pour nos lignes 1, 3 et 4 : première occurence (nnn K€) Aide PIA, Montant du projet (nnn K€)
ligns_part %>% 
  mutate(
    Montant_aide = sapply(str_extract_all(ligns_part$`FINANCÉ PAR`, "\\d{3,4} K\\€"), function(x) if(length(x) >= 1) x[1]),
    Montant_proj = sapply(str_extract_all(ligns_part$`FINANCÉ PAR`, "\\d{3,4} K\\€"), function(x) if(length(x) >= 2) x[2]),
    `DONT AIDE PIA` = ifelse(is.na(`DONT AIDE PIA`), Montant_aide, `DONT AIDE PIA`)
    )
  

# Préparer l'extraction de ces informations
str_extract_all(ligns_part[1, ]$`FINANCÉ PAR`, "\\d{3,4} K\\€")

## Pour la colonne réalisation
ligns_part[1, 2]
ligns_part[1, 2] = paste(str_extract_all(ligns_part[1, 2], "\\d{4}")[[1]][1], str_extract_all(ligns_part[1, 2], "\\d{4}")[[1]][2], sep=" - ")


realisation_year = str_extract_all(dataset_inf_proj_1_3$RÉALISATION, "\\d{4}")
premiere_periode = sapply(realisation_year, function(x) x[1])
deuxieme_periode = sapply(realisation_year, function(x) x[2])

deuxieme_periode

# On peut observer qu'on a pas de valeur manquante pour la colonne Réalisation
na_realisation = dataset_inf_proj_1_3 %>% 
                filter(is.na(RÉALISATION))
na_realisation

# Plus tard
#DEPT = str_extract_all(LOCALISATION, "\\(\\d{2,3}", simplify = TRUE)